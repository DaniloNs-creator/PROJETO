...
# ==============================================================================
# PARTE 3: PARSER APP 2 E SIGRAWEB
# ==============================================================================
...
class SigrawebPDFParser:
    """
    Parser otimizado para o layout SIGRAWEB (Conferência do Processo Detalhado).
    Extrai as informações de cada Adição mapeando perfeitamente para a DUIMP.
    """
    ...
    def parse_pdf(self, pdf_path: str) -> Dict:
        try:
            logger.info(f"Iniciando parsing do layout SIGRAWEB: {pdf_path}")
            text_chunks = []
    ...
    def _process_full_text(self, text: str):
        chunks = re.split(r'(Informações da Adição Nº:\s*\d+)', text, flags=re.IGNORECASE)
        items_found = []
        ...
        if len(chunks) > 1:
            for i in range(1, len(chunks), 2):
                ...
                content = chunks[i+1]
                item_num_match = re.search(r'(\d+)', header)
                item_num = int(item_num_match.group(1)) if item_num_match else (i // 2 + 1)
                item_data = self._parse_item_block(item_num, content)
                if item_data:
                    items_found.append(item_data)
        else:
            st.warning("⚠️ O sistema não detectou o padrão 'Informações da Adição Nº:'. Verifique o PDF SIGRAWEB.")
    ...
    def _parse_item_block(self, item_num: int, text: str) -> Dict:
        try:
            ...
            # Identificadores
            code_match = re.search(r'Part Number:\s*([\w\.\-]+)', text, re.IGNORECASE)
            if code_match: item['codigo_interno'] = code_match.group(1).replace('.', '')

            ncm_match = re.search(r'NR NCM:\s*(\d+)', text, re.IGNORECASE)
            if ncm_match: item['ncm'] = ncm_match.group(1)

            # Valores e Pesos
            peso_match = re.search(r'Peso Líquido:\s*([\d\.,]+)', text, re.IGNORECASE)
            if peso_match: item['peso_liquido'] = self._parse_valor(peso_match.group(1))

            frete_match = re.search(r'Valor Frete Real:\s*([\d\.,]+)', text, re.IGNORECASE)
            if frete_match: item['frete_internacional'] = self._parse_valor(frete_match.group(1))

            seg_match = re.search(r'Valor Seguro Real:\s*([\d\.,]+)', text, re.IGNORECASE)
            if seg_match: item['seguro_internacional'] = self._parse_valor(seg_match.group(1))

            adu_match = re.search(r'Valor Aduaneiro Real:\s*([\d\.,]+)', text, re.IGNORECASE)
            if adu_match: item['local_aduaneiro'] = self._parse_valor(adu_match.group(1))

            # --- LEITURA DA TABELA DE IMPOSTOS (Lógica de blocos para fugir de Regex frágil) ---
            block_match = re.search(r'Base de Cálculo\s*\|\s*Valor(.*?)(?:ICMS|Item N°:)', text, re.DOTALL | re.IGNORECASE)
            if block_match:
                tax_block = block_match.group(1)
                rows = [row for row in tax_block.split('\n') if '|' in row]
                
                for r in rows:
                    cols = [c.strip() for c in r.split('|')]
                    # Garante que temos as 7 colunas esperadas (ou 8 contando o índice inicial vazio)
                    if len(cols) >= 7:
                        aliq = self._parse_valor(cols[1])
                        base = self._parse_valor(cols[-2])
                        valor = self._parse_valor(cols[-1])
                        
                        label = cols[0].strip().upper()
                        
                        if 'IPI' in label:
                            item['ipi_aliquota'] = aliq
                            item['ipi_base_calculo'] = base
                            item['ipi_valor_devido'] = valor
                        elif 'PIS' in label:
                            item['pis_aliquota'] = aliq
                            item['pis_base_calculo'] = base
                            item['pis_valor_devido'] = valor
                        elif 'COFINS' in label:
                            item['cofins_aliquota'] = aliq
                            item['cofins_base_calculo'] = base
                            item['cofins_valor_devido'] = valor
                        elif label == '': # O II geralmente é a primeira linha e não tem label na frente
                            item['ii_aliquota'] = aliq
                            item['ii_base_calculo'] = base
                            item['ii_valor_devido'] = valor

            return item
            
        except Exception as e:
            logger.error(f"Erro item {item_num} SIGRAWEB: {e}")
            return None
...
def sistema_integrado_duimp():
    ...
    with tab1:
        ...
        with col2:
            st.markdown('<div class="section-card">', unsafe_allow_html=True)
            st.info("Passo 2: Carregue o Relatório Auxiliar")
            tipo_app2 = st.radio("Formato do PDF Auxiliar:", ["Extrato DUIMP (Padrão)", "SIGRAWEB (Conferência)"], horizontal=True)
            file_hafele = st.file_uploader("Arquivo Auxiliar (.pdf)", type="pdf", key="u2")
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        # Processamento Arquivo Auxiliar (APP 2 / SIGRAWEB)
        if file_hafele and st.session_state["parsed_hafele"] is None:
            ...
            try:
                if tipo_app2 == "Extrato DUIMP (Padrão)":
                    parser_h = HafelePDFParser()
                else:
                    parser_h = SigrawebPDFParser()
                ...
                if qtd_itens > 0:
                    st.markdown(f'<div class="success-box">✅ Arquivo Auxiliar Lido com Sucesso! {qtd_itens} itens encontrados.</div>', unsafe_allow_html=True)
                else:
                    st.warning("O PDF foi lido, mas nenhum item foi identificado automaticamente. Verifique se o layout correto foi selecionado.")
                    
            except Exception as e:
                st.error(f"Erro ao ler arquivo auxiliar: {e}")
            finally:
